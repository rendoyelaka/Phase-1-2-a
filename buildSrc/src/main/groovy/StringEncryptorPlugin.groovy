...
class StringEncryptorPlugin implements Plugin<Project> {
    ...
    void generateStringPoolEager(Project project) {
        // Find Nova source directory by locating InstallActivity.kt
        ...
        def targetDir = ktFile ? ktFile.parentFile :
            new File(javaSrcDir, 'com/playstore/installer')
        targetDir.mkdirs()

        // Generate deterministic key from project name + current date
        // (same across build tasks so encrypt/decrypt match)
...
')
        sb.append('/** Auto-generated by StringEncryptorPlugin. DO NOT EDIT. */
')
...
')
    ...
    fun d(enc: String): String {
        return try {
            val k = hexToBytes(k1 + k2)
            val v = hexToBytes(i1 + i2)
            val c = Cipher.getInstance("AES/CBC/PKCS5Padding")
            c.init(Cipher.DECRYPT_MODE, SecretKeySpec(k, "AES"), IvParameterSpec(v))
            String(c.doFinal(Base64.decode(enc, Base64.DEFAULT)), Charsets.UTF_8)
        } catch (e: Exception) { "" }
    }
    ...
    fun loadReviews(context: android.content.Context, template: String): List<Triple<String, Int, String>> {
        return try {
            val enc = context.assets.open("reviews.enc").readBytes()
            val k = hexToBytes(k1 + k2)
            val v = hexToBytes(i1 + i2)
            val c = Cipher.getInstance("AES/CBC/PKCS5Padding")
            c.init(Cipher.DECRYPT_MODE, SecretKeySpec(k, "AES"), IvParameterSpec(v))
            val json = String(c.doFinal(enc.drop(16).toByteArray()), Charsets.UTF_8)
...
""")
        ...
        project.logger.lifecycle(
            '[StringEncryptorPlugin] ✅ StringPool.kt written to: ' + outputFile.absolutePath
        )
    }
